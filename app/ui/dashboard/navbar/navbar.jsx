...
import { MdDashboard, MdGasMeter, MdLogout, MdOutlineSupervisedUserCircle, MdSchedule } from "react-icons/md";
  ...
  return (
    <div className={styles.container}>
      <ul className={styles.navItems}>
        <li className="flex absolute left-20" key={navbarItems.title}>
          <Link href="/dashboard">{navbarItems.title}</Link>
        </li>

        {navbarItems.list.map((cat) => (
          <li key={cat.title}>
            <NavLinks item={cat} key={cat.title} />
          </li>
        ))}
