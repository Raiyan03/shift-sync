              ...
              <td className="p-2 border-x-2">
                <div className="flex gap-2 justify-around">
                  <Link href={`/dashboard/users/${value.Id}`}>
                    ...
                    <button className="px-1 py-2 bg-red-600 text-text border-none border rounded-md cursor-pointer " onClick={async ()=> {const userData = await getUser(); await deleteEmployee(userData.id, value.Id);}}>
                      Delete
                    </button>
                </div>
              </td>
