                ...
                <td className="p-2 ">
                  <div className="flex gap-2 justify-around">
                    <Link href={`/manager/employees/${value.Id}`}>
                      ...
                      <button className="px-1 py-2 bg-accent2 text-text border-none  rounded-md cursor-pointer " onClick={async ()=> {const userData = await getUser(); await deleteEmployee(userData.id, value.Id);}}>
                        Delete
                      </button>
                  </div>
                </td>
